...
class PoisonedCIFAR10(CIFAR10, AddCIFAR10TriggerMixin):
    def __init__(self, benign_dataset, y_target, poisoned_rate, poisoned_transform_index, \
                 ...
                 sigma=-1, ghost_alpha=-1., poisoned_set=None):
        ...
        print(f"Poisoned set size: {len(self.poisoned_set)}")
        # Add trigger to images
        if self.transform is None:
            self.poisoned_transform = Compose([])
        else:
...
def CreatePoisonedDataset(benign_dataset, *args, **kwargs):
    # class_name = type(benign_dataset)
    # if class_name == DatasetFolder:
    #     return PoisonedDatasetFolder(benign_dataset, y_target, poisoned_rate, poisoned_transform_index,
    #                                  poisoned_target_transform_index, reflection_cadidates, \
    #                                  max_image_size=max_image_size, ghost_rate=ghost_rate, alpha_b=alpha_b,
    #                                  offset=offset, sigma=sigma, ghost_alpha=ghost_alpha)
    # elif class_name == CIFAR10:
    return PoisonedCIFAR10(benign_dataset, *args, **kwargs)
    # elif class_name == MNIST:
    #     return PoisonedMNIST(benign_dataset, y_target, poisoned_rate, poisoned_transform_index,
    #                          poisoned_target_transform_index, reflection_cadidates, \
    #                          max_image_size=max_image_size, ghost_rate=ghost_rate, alpha_b=alpha_b, offset=offset,
    #                          sigma=sigma, ghost_alpha=ghost_alpha)
    # else:
