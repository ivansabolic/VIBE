...
class AddTrigger:
    ...
    def add_trigger(self, img, pattern, alpha):
...
class AddCIFAR10Trigger(AddTrigger):
    ...
    def __call__(self, img):
        ...
        img = np.array(img)
        img = self.add_trigger(img, self.pattern, self.alpha)
...
class PoisonedCIFAR10(CIFAR10):
    def __init__(self,
        ...
        if poisoned_set and Path(poisoned_set).exists():
            ...
            if poisoned_rate == 1:
                self.poisoned_set = torch.arange(total_num)
            else:
        ...
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddCIFAR10Trigger(delta, freq, alpha=0.2))
...
class PoisonedGTSRB(GTSRB):
    def __init__(self,
        ...
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddGTSRBTrigger(pattern, weight))
...
class SIG(Base):
    """Construct poisoned datasets with SIG method.
