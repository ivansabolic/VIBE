...
class Base(object):
    ...
    def train(self, schedule=None):
        ...
        elif self.current_schedule['defense'] == '':
            exp_info = 'poisoned_training'
        else:
            if 'load_pretrained' in self.current_schedule and self.current_schedule['load_pretrained'] is True:
                exp_info = 'defense_pretrained'
            elif 'ransac' in self.current_schedule['defense']:
                exp_info = 'defense_ransac'
            else:
                ind_name = self.current_schedule['defense'][self.current_schedule['defense'].index("indices_")+len("indices_"):-4]
                if 'projections' in self.current_schedule['defense'].split('/')[-2]:
                    ind_name += '_projections'
                if self.current_schedule['defense'].split('/')[-2].endswith('retrain'):
                    ind_name += '_retrain'
                exp_info = f'defense_{ind_name}'
        ...
        len_dataset = len(self.train_dataset) if self.current_schedule['benign_training'] is True else len(self.poisoned_train_dataset)
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            msg = time.strftime("[%Y-%m-%d_%H:%M:%S] ", time.localtime()) + f"Epoch:{i+1}/{self.current_schedule['epochs']}, lr: {self.current_schedule['lr']}, loss: {float(loss)}, time: {time.time()-last_time}\n"
            last_time = time.time()
            log(msg)
            if self.current_schedule['wandb'] is True:
            ...
            if (i + 1) % self.current_schedule['test_epoch_interval'] == 0:
                ...
                log(msg)
                if self.current_schedule['wandb'] is True:
                    wandb.log({'benign_top1_acc': top1_correct/total_num, 'benign_top5_acc': top5_correct/total_num}, step=i+1)
                ...
                log(msg)
                if self.current_schedule['wandb'] is True:
                    wandb.log({'poisoned_top1_acc': top1_correct/total_num, 'poisoned_top5_acc': top5_correct/total_num}, step=i+1)
